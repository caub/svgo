...
function convertTransform(item, attrName, params) {
    ...
    if (!params.hasOwnProperty('degPrecision')) {
        params.degPrecision = ;
    }
    var degPrecision = Math.pow(10, params.degPrecision || Math.max(0, Math.min(params.floatPrecision, significantDigits - 2)));
    var floatPrecision = Math.pow(10, params.floatPrecision);
    var transformPrecision = Math.pow(10, params.floatPrecision);

    params.floatRound = floatPrecision==1 ? round : smartRound.bind(this, floatPrecision);

    params.degRound = degPrecision == 1 ? round : smartRound.bind(this, params.degPrecision);
...
function roundTransform(transform, params) {
    switch (transform.name) {
        case 'translate':
            transform.data = params.floatRound(transform.data);
            break;
        case 'rotate':
            transform.data = degRound(transform.data.slice(0, 1)).concat(params.floatRound(transform.data.slice(1)));
            break;
        case 'skewX':
        case 'skewY':
            ...
            break;
        case 'scale':
            transform.data = params.transformRound(transform.data);
            break;
        case 'matrix':
            transform.data = params.transformRound(transform.data.slice(0, 4)).concat(params.floatRound(transform.data.slice(4)));
            break;
    }
    return transform;
}
